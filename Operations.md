...
## Independent refresh and defense matchups (September 23, 2026)

Research now rebuilds from the latest retained Yahoo snapshot every two hours, even if the Mac has not delivered a new import. Current-season statistical files and schedules are eligible for source refresh every two hours; prior-season files remain cached for one day. Failed research collection retries after a minimum 15-minute backoff. Stalled analysis leases can recover after ten minutes. Existing source fallback keeps last-good statistical data and the saved Qwen history; freshness timestamps continue to identify their actual capture/generation times. The Mac is still required for browser-based Yahoo collection and the existing Qwen worker. A public-data refresh does not imply newly verified Yahoo roster membership.

Dashboard requests now time out so a hung connection cannot permanently hold the polling lock. DEF role labels in player tables and dossiers include the next uncompleted regular-season opponent and that offense's fantasy points allowed per game to opposing defenses under the imported league scoring. The calculation uses completed pre-snapshot-week team statistical rows, shows season/sample count, and explicitly labels a prior-season fallback when current-season history is absent. Missing data shows unavailable; it is never converted to a zero average.

Release validation: production build and 129 tests pass, including defense allowance orientation, future-game exclusion, zero scores, missing data, and prior-season fallback. Source/database backups and the prior web image are retained under `/var/backups/fantasy-football-edge/refresh-defense-20260923` and image tag `fantasy-edge-v2-web:before-refresh-defense-20260923`. Only the web service was recreated; the database and other VPS services were preserved.

## Position shortlists and DEF matchup projection (September 23, 2026)

Waivers now has three separately ranked candidates for each of QB, RB, WR, TE, K and DEF. The old QB-first combined list and duplicate position board were replaced by one shared shortlist. Every dashboard update reranks imported candidates, excluding locked/completed games, byes, unavailable designations and players outside FA/waiver availability. Non-DEF forecasts use compatible fresh Qwen points or explicitly labeled Yahoo fallback; old Qwen priority scores no longer determine this shortlist.

The DEF model scores each completed current-season team game with the league's configured defensive scoring. Its point estimate is `0.5 × defense's current-season fantasy points/game + 0.5 × next opponent's fantasy points allowed/game to opposing defenses`. Both samples must exist, scoring must be supported, and the next matchup must belong to the snapshot week. Prior-season games and future/same-week results are excluded from this model. A missing sample yields no matchup estimate, preserving a labeled Qwen/Yahoo alternative; supported DEF estimates are ranked before fallback-only entries. The equal weights are a transparent heuristic, not fitted/calibrated accuracy. Small samples are explicitly marked volatile. Rows expose each included game, both means, the formula, research time and nflverse source. Yahoo and Qwen columns elsewhere remain their own forecasts.

Research schema version 14 triggers recalculation on rollout; the existing two-hour independent refresh then maintains the inputs. The server’s Qwen candidate selection also considers these DEF estimates. Production build and 133 tests pass, including position coverage, availability filters, current-season exclusion rules, zero/negative points, absent samples, scoring completeness and model-driven DEF order. Rollback artifacts: `/var/backups/fantasy-football-edge/waiver-matchups-20260923/` and image `fantasy-edge-v2-web:before-waiver-matchups-20260923`.
