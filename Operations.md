...
During the final backfill, the resident Qwen model had a 1,024-token context. Small inference requests succeeded while the worker's existing 8,192-token requests timed out; `/api/ps` alone did not establish inference health. Unloading the resident model through Ollama and sending an 8,192-token request reloaded the same installed model at the required size. The probe then completed in about 2.5 seconds and real projection jobs resumed. No model files, service configuration, production routing or credentials were changed. Superseded inputs were retired and affected current jobs were requeued; rejected outputs were never published. Investigate loaded context and test actual inference when health/list endpoints pass but generation stalls.
