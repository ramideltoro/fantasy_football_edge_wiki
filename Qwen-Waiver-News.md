...
Research version 4 rebuilds the latest intelligence record on rollout; future snapshots use the same automatic research/worker flow. The schema-constrained response adds waivers alongside the existing team priorities and player insights. Worker uses a bounded 16K context and 1,600 output tokens.
